...
parser = argparse.ArgumentParser(description="Run FastAPI app with common directory.")
parser.add_argument(
    "--common_dir",
    type=str,
    required=True, 
    help="Directory to store or serve common files"
)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    save_path = os.path.join(COMMON_DIR, file.filename)
...
async def websocket_endpoint(ws: WebSocket):
    ...
    if mode == "connect":
        ...
        zip_path = os.path.join(COMMON_DIR, zip_name)
...
async def download_file(filename: str):
    file_path = os.path.join(COMMON_DIR, filename)
